...
def csv_to_parquet(path: str, output: str):
    ...
    for file in os.listdir(path):
        if file.endswith('CSV'):
            ...
            table = pv.read_csv(os.path.join(path, new_file),parse_options = options)
            pq.write_table(table, os.path.join(output,new_file.replace('csv', 'parquet')))
